...
: describe("test fundme contract", async function() {
    ...
    it("window closed, value greater than minium, fund failed", async function() {
        ...
        // vaule is greater minimum value
        await expect(fundMe.fund({value: ethers.parseEther("0.1")}))
            .to.be.revertedWith("window is closed");
    })

    it("window open, value less than minium, fund failed", async function() {
        await expect(fundMe.fund({value: ethers.parseEther("0.01")}))
            .to.be.revertedWith("Send more ETH");
    })

    it("window open, value greater than minium, fund success", async function() {
        // vaule is greater minimum value
    ...
    it("window closed, target not reached, funder has balance", async function() {
        ...
        await expect(fundMe.refund()).to.be.emit(fundMe, "RefundbyFunder").withArgs(firstAccount, ethers.parseEther("0.1"));
    })
})